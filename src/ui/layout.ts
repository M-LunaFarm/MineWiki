...
export function layout(title: string, body: string, user: CurrentUser | null = null, currentSpace = '', seo: SeoOptions = {}) {
  ...
  const mobileSearch = `<details class="mobile-search">
      <summary aria-label="검색">검색</summary>
  ...
  const userAccount = user
    ? `<span class="user-chip" title="${escapeHtml(userRole.title)}"><span class="user-role">${escapeHtml(userRole.label)}</span> <a href="/me">${escapeHtml(user.display_name)}</a></span>`
    : '<a href="/join">가입</a><a href="/login">로그인</a>';
  const publicNav = `${nav('/wiki', '위키', 'main')}
      ${nav('/mods', '모드', 'mod')}
      ${nav('/servers', '서버', 'server')}
      ${nav('/dev', '개발', 'dev')}
      <span class="nav-separator"></span>
      <a href="/recent">최근 바뀜</a>
      <a href="/new">새 문서</a>
      ${userWorkbench}
      <span class="mobile-menu-section">계정</span>
      ${userAccount}
      ${themeToggle}`;
  const adminNav = `<span class="admin-mode-chip">관리자 모드</span>
      <span class="mobile-menu-section">관리</span>
      <a href="/admin">운영자 홈</a>
      <a href="/admin/recent">최근 바뀜</a>
      <a href="/admin/reports">신고</a>
      <a href="/admin/work">검토 큐</a>
      <a href="/admin/jobs">작업 큐</a>
      <a href="/admin/imports">이전 작업</a>
      <a href="/admin/subwikis">서브위키</a>
      <a href="/admin/audits">감사</a>
      <a href="/admin/identity">사용자/권한</a>
      <a href="/admin/filters">편집 필터</a>
      <a href="/admin/publication">공개 운영</a>
      <a href="/admin/mod-verification">모드 검증</a>
      <a href="/admin/files">파일</a>
      <span class="mobile-menu-section">사이트</span>
      <a href="/wiki">사이트로 돌아가기</a>
      <span class="mobile-menu-section">계정</span>
      ${userAccount}
      ${themeToggle}`;
  const navHtml = isAdminLayout ? adminNav : publicNav;
  const desktopSearchHtml = isAdminLayout ? '' : `<form class="top-search desktop-search" action="/search" method="get">
      <input name="q" placeholder="검색" autocomplete="off">
      <button>검색</button>
    </form>`;
  const mobileSearchHtml = isAdminLayout ? '' : mobileSearch;
  ...
  const intentStrip = pageIntentStrip(title, currentSpace, user, isAdminLayout);
  return `<!doctype html>
<html lang="ko">
<head>
  <meta charset="utf-8">
  ...
  <link rel="stylesheet" href="/assets/wiki-skin.css?v=20260526-recent-dark-contrast">
  ${seo.headHtml ?? ''}
