...
test('subwiki sidebars and article TOC preserve parent child hierarchy', () => {
  ...
  assert.equal(html.includes('class="page-intent-strip'), false);
  assert.equal(html.includes('desktop-inline-toc'), true);
  assert.equal(html.includes('mobile-inline-toc'), true);
  assert.equal(html.includes('<aside class="article-toc"'), false);
...
test('auth pages use shared wiki chrome and flat wiki skin tokens', () => {
  ...
  assert.equal(html.includes('/assets/wiki-skin.css?v=20260621-document-first'), true);
...
test('layout emits MineWiki SEO and icon metadata', () => {
  ...
  assert.equal(html.includes('/assets/wiki-skin.css?v=20260621-document-first'), true);
  assert.equal(html.includes('/assets/styles/tokens.css?v=20260621-document-first'), true);
  assert.equal(html.includes('/assets/styles/layout.css?v=20260621-document-first'), true);
  assert.equal(html.includes('/assets/styles/article.css?v=20260621-document-first'), true);
  assert.equal(html.includes('/assets/styles/pages/front-page.css?v=20260621-document-first'), true);
  assert.equal(html.includes('class="page-intent-strip" aria-label="현재 화면 바로가기"'), true);
  ...
  const articleChromeHtml = layout('문서 테스트', '<main class="wiki-shell skin-article"></main>', null, 'main', { hideIntentStrip: true });
  assert.equal(articleChromeHtml.includes('class="page-intent-strip'), false);
});
