import { BadRequestException, Body, Controller, Delete, Get, Param, ParseIntPipe, Patch, Post, Query, Req, Res, ServiceUnavailableException, UseGuards } from '@nestjs/common';
...
import { readWikiAnonymousContributorToken, serializeWikiAnonymousContributorCookie } from './wiki-anonymous-contributor';
...
export class WikiDiscussionController {
  ...
  permissions(@Param('pageId') pageId: string, @Req() request: FastifyRequest): Promise<{ readonly canCreateThread: boolean }> {
    return this.discussions.getPageDiscussionPermissions(pageId, request.sessionPayload ?? null, request.clientIp);
  }
  ...
  async create(
    ...
    if (!session) {
      this.discussions.assertAnonymousDiscussionsEnabled();
      if (!this.wikiCaptcha.isRequired()) {
        throw new ServiceUnavailableException('Anonymous wiki discussion CAPTCHA is unavailable.');
      }
      if (!request.clientIp) throw new ServiceUnavailableException('Validated client address is unavailable.');
    }
    await this.wikiCaptcha.assertVerified(body.captchaToken, request.clientIp ?? session?.requestIp);
    const thread = { ...body };
    delete thread.captchaToken;
    if (session) return this.discussions.createThread(session, pageId, thread);
    const result = await this.discussions.createAnonymousThread(
      pageId,
      thread,
      request.clientIp!,
      readWikiAnonymousContributorToken(request.headers?.cookie),
    );
    if (result.ownerTokenIssued) reply.header('Set-Cookie', serializeWikiAnonymousContributorCookie(result.ownerToken));
  ...
  async comment(
    @Param('threadId') threadId: string,
    @Body() body: { content?: string; poll?: WikiDiscussionPollInput; captchaToken?: string },
    @Req() request: FastifyRequest,
    @Res({ passthrough: true }) reply: FastifyReply,
  ) {
    const session = request.sessionPayload ?? null;
    if (session) {
      ...
      return this.discussions.addComment(session, threadId, authenticatedBody);
    }
    this.discussions.assertAnonymousDiscussionsEnabled();
    if (!this.wikiCaptcha.isRequired()) throw new ServiceUnavailableException('Anonymous wiki discussion CAPTCHA is unavailable.');
    if (!request.clientIp) throw new ServiceUnavailableException('Validated client address is unavailable.');
    await this.wikiCaptcha.assertVerified(body.captchaToken, request.clientIp);
    const anonymousBody = { ...body };
    delete anonymousBody.captchaToken;
    const result = await this.discussions.addAnonymousComment(
      threadId,
      anonymousBody,
      request.clientIp,
      readWikiAnonymousContributorToken(request.headers?.cookie),
