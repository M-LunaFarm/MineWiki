...
test('route resolver batch-loads namespaces and server slugs without cross-space collisions', async () => {
  ...
  const prisma = {
    ...
    serverWiki: {
      async findMany() {
        serverWikiQueries += 1;
        return [{ spaceId: 10n, slug: 'alpha' }, { spaceId: 20n, slug: 'beta' }];
      }
    }
  ...
  assert.equal(routes.routePath(pages[0]), '/server/alpha/%EA%B7%9C%EC%B9%99');
  assert.equal(routes.routePath(pages[1]), '/server/beta/%EA%B7%9C%EC%B9%99');
...
test('route resolver reuses known namespaces and contextualizes unresolved server targets', async () => {
  ...
  const prisma = {
    ...
    serverWiki: {
      async findMany() { return [{ spaceId: 10n, slug: 'alpha' }]; }
    }
  } as unknown as PrismaService;
  ...
  assert.equal(routes.targetRoutePath('server', 'alpha/없는 문서', page), '/server/alpha/%EC%97%86%EB%8A%94_%EB%AC%B8%EC%84%9C');
