    ...
    <div className="mx-auto w-full max-w-6xl space-y-7">
      ...
      <section>
        ...
        <div className="divide-y divide-white/10 border border-white/10 bg-[#111821]">
          {result.items.map((item) => <Link key={item.id} href={item.routePath} className="flex items-center justify-between gap-4 p-4 transition hover:bg-white/[0.03] sm:p-5"><div className="min-w-0"><p className="truncate font-semibold text-white">{item.displayTitle}</p><p className="mt-1 truncate text-xs text-slate-500">{item.namespace}:{item.title}</p></div><div className="flex shrink-0 items-center gap-3 text-xs text-slate-500">{item.value !== null ? <span>{type === 'wanted' ? `링크 ${item.value}` : `${item.value.toLocaleString('ko-KR')} bytes`}</span> : null}<ArrowUpRight className="size-4" /></div></Link>)}
