...
export default async function WikiEditRequestQueuePage({ searchParams }: PageProps) {
  ...
  const scope = query.scope === 'mine' || query.scope === 'reviewable' ? query.scope : 'all';
  ...
  return <section className="mx-auto w-full max-w-5xl space-y-7">
    ...
    <form method="get" className="surface-flat grid gap-3 p-4 sm:grid-cols-3 sm:p-5">
      ...
      <label className="grid gap-2 text-xs font-semibold text-slate-400">범위<select name="scope" defaultValue={scope} className="input min-h-11"><option value="all">볼 수 있는 모든 요청</option><option value="reviewable">내가 검토할 요청</option><option value="mine">내 요청</option></select></label>
