...
export default async function ModpackWikiPage({ params }: PageProps) {
  ...
  if (toolRoute?.tool === 'edit') return <WikiEditRoutePage prefix="modpack" segments={toolRoute.documentSegments} />;
  if (toolRoute?.tool === 'history') return <WikiHistoryRoutePage prefix="modpack" segments={toolRoute.documentSegments} />;
