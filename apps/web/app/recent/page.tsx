...
export default async function RecentChangesPage({ searchParams }: RecentChangesPageProps) {
  ...
  return (
    <main className="wiki-list-page mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      <header className="border-b border-white/10 pb-6">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-300">Wiki activity</p>
        <h1 className="mt-2 text-3xl font-bold tracking-[-0.025em] text-white sm:text-4xl">최근 변경</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">읽을 수 있는 문서의 공개 변경 기록을 시간순으로 확인하고, 이름공간과 변경 유형으로 좁혀보세요.</p>
      </header>
      <form className="surface-flat grid gap-3 rounded-xl p-4 sm:grid-cols-3 lg:grid-cols-[1fr_1fr_1fr_auto]" action="/recent">
