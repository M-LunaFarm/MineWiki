...
export default async function DataWikiPage({ params }: PageProps) {
  ...
  if (path.at(-1) === 'edit') return <WikiEditRoutePage prefix="data" segments={path.slice(0, -1)} />;
  if (path.at(-1) === 'history') return <WikiHistoryRoutePage prefix="data" segments={path.slice(0, -1)} />;
