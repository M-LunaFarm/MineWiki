...
export function ServerWikiReleaseReviewDetailClient({ candidateId }: { readonly candidateId: string }) {
  ...
  useEffect(() => { void load(); }, [baseUrl, candidateId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function changeApproval(approve: boolean) {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.message ?? '승인 상태를 변경하지 못했습니다.');
  ...
  const changedPages = detail.manifest.pages.filter((page) => page.kind !== 'unchanged');
