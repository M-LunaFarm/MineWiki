...
import { fetchWikiPageAclHistoryEvents, fetchWikiPageByPath, fetchWikiPageLifecycleEvents, fetchWikiRevisions } from '../../lib/wiki-server-api';
...
export async function WikiHistoryRoutePage({ prefix, segments = [] }: WikiHistoryRoutePageProps) {
  ...
  if (!page) notFound();
  const [revisions, lifecycle, aclHistory] = await Promise.all([
    fetchWikiRevisions(page.id),
    fetchWikiPageLifecycleEvents(page.id),
    fetchWikiPageAclHistoryEvents(page.id)
  ]);
  const history = <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
    ...
    <WikiHistoryListClient pageId={page.id} currentRevisionId={page.revision.id} routePath={routePath} initial={revisions} initialLifecycle={lifecycle} initialAclHistory={aclHistory} />
  </div>;
