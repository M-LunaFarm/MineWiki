...
import { fetchWikiPageByPath, fetchWikiPageLifecycleEvents, fetchWikiRevisions } from '../../lib/wiki-server-api';
...
export async function WikiHistoryRoutePage({ prefix, segments = [] }: WikiHistoryRoutePageProps) {
  ...
  const history = <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
    ...
    <WikiHistoryListClient pageId={page.id} currentRevisionId={page.revision.id} routePath={routePath} initial={revisions} initialLifecycle={lifecycle} />
  </div>;
