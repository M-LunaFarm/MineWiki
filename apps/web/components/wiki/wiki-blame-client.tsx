...
export function WikiBlameClient({ pageId, returnTo, revisionId }: { readonly pageId: string; readonly returnTo: string; readonly revisionId?: string }) {
  const [data, setData] = useState<WikiBlameResponse | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let active = true;
    void fetchWikiBlame(pageId, revisionId)
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
      ...
      <header className="border-b border-white/10 pb-6"><h1 className="flex items-center gap-3 text-3xl font-bold text-white"><GitCommitHorizontal className="size-7 text-emerald-300" /> 행별 기여 기록</h1><p className="mt-3 text-sm text-slate-400">{revisionId ? '선택한 판' : '현재 공개 판'}의 각 행이 마지막으로 변경된 리비전과 작성자를 추적합니다.</p></header>
      ...
      {data ? <>
        <div className="flex flex-wrap gap-2 text-xs"><span className="chip chip-muted">대상 rev {data.revisionNo}</span><span className="chip chip-muted">분석 리비전 {data.revisionCount}</span><span className="chip chip-muted">원문 {data.lineCount.toLocaleString('ko-KR')}행</span></div>
