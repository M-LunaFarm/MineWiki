...
export function WikiUserAdmin() {
  ...
  async function confirmChange(event: FormEvent) {
    ...
    try {
      const updated = await setWikiAdminUserBlocked({ profileId: pending.user.id, blocked: pending.blocked, reason, publicReason });
