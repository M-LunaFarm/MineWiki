...
export function WikiUserAdmin() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-3">
      {users.map((user) => <article key={user.id} className="flex flex-col gap-4 border border-white/10 bg-[#111821] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Link href={`/wiki/contributions/${user.id}`} className="font-semibold text-white hover:text-emerald-200">{user.displayName}</Link><span className={`chip ${user.status === 'blocked' ? 'border-red-300/30 text-red-200' : 'chip-muted'}`}>{user.status}</span></div><p className="mt-1 break-all text-xs text-slate-500">@{user.username} · #{user.id}</p></div>{user.status === 'active' || user.status === 'blocked' ? <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">{user.status === 'blocked' ? <Link href={`/admin/wiki/batch-rollback?targetProfileId=${encodeURIComponent(user.id)}&username=${encodeURIComponent(user.username)}`} className="btn-primary min-h-11 w-full sm:w-auto"><RotateCcw className="size-4" /> 기여 복구</Link> : null}<button type="button" onClick={() => { setPending({ user, blocked: user.status === 'active' }); setReason(''); }} className="btn-secondary min-h-11 w-full sm:w-auto">{user.status === 'active' ? <><Ban className="size-4" /> 차단</> : <><RotateCcw className="size-4" /> 해제</>}</button></div> : null}</article>)}
