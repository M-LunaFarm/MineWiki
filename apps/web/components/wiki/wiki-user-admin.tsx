...
export function WikiUserAdmin() {
  ...
  async function confirmChange(event: FormEvent) {
    ...
    try {
      const updated = await setWikiAdminUserBlocked({ profileId: pending.user.id, blocked: pending.blocked, reason });
      setUsers((current) => current.map((user) => user.id === updated.id ? updated : user));
      setEvents(await fetchWikiUserBlockEvents());
  ...
  return <div className="space-y-6">
    <header className="border border-white/10 bg-white/[0.03] p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="flex items-center gap-2 text-sm font-semibold text-emerald-200"><ShieldCheck className="size-4" /> Wiki Admin</p><h1 className="mt-2 text-2xl font-semibold text-white">사용자 기여 차단</h1><p className="mt-2 text-sm text-slate-400">계정 자체가 아니라 위키 편집·토론·문서 생성 권한만 차단합니다.</p></div><nav className="flex flex-wrap gap-2"><Link href="/admin/wiki" className="chip chip-muted">최근 변경</Link><Link href="/admin/wiki/pages" className="chip chip-muted">문서</Link><Link href="/admin/wiki/acl" className="chip chip-muted">ACL</Link><span className="chip chip-accent">사용자 차단</span></nav></div>
    </header>
    ...
    <section className="grid gap-3">
      {users.map((user) => <article key={user.id} className="flex flex-col gap-4 border border-white/10 bg-[#111821] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Link href={`/wiki/contributions/${user.id}`} className="font-semibold text-white hover:text-emerald-200">{user.displayName}</Link><span className={`chip ${user.status === 'blocked' ? 'border-red-300/30 text-red-200' : 'chip-muted'}`}>{user.status}</span></div><p className="mt-1 break-all text-xs text-slate-500">@{user.username} · #{user.id}</p></div>{user.status === 'active' || user.status === 'blocked' ? <button type="button" onClick={() => { setPending({ user, blocked: user.status === 'active' }); setReason(''); }} className="btn-secondary min-h-11 w-full sm:w-auto">{user.status === 'active' ? <><Ban className="size-4" /> 차단</> : <><RotateCcw className="size-4" /> 해제</>}</button> : null}</article>)}
