...
interface OAuthFlowStatusProps {
  ...
  readonly state: Extract<OAuthProviderChoiceState, 'pending' | 'success' | 'error'>;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3" aria-label="간편 로그인 공급자">
        <OAuthProviderChoice
          provider="discord"
          state={provider === 'discord' ? state : 'inactive'}
        />
        <OAuthProviderChoice
          provider="naver"
          state={provider === 'naver' ? state : 'inactive'}
        />
      </div>

      <div
        className={`flex min-h-[5.5rem] items-start gap-3 rounded-lg border px-3.5 py-3 ${
          state === 'error'
