...
export function ServerOwnershipTransferInbox() {
  ...
  async function respond(item: Transfer, action: 'accept' | 'decline') {
    ...
    try {
      ...
      await load();
    } catch (value) { setError(message(value, '소유권 이전 요청에 응답하지 못했습니다.')); await load(); }
  ...
  return (
    <PrivilegedActionGate purpose="server_ownership_transfer" title="소유권 이전 요청 확인" description="서버 전체 권한 수락 또는 거절은 전용 다중 인증으로 보호됩니다." onUnlocked={load}>
      <section id="server-ownership-transfers" aria-labelledby="server-ownership-transfers-title" aria-busy={loading || workingId !== null} className="mb-6 scroll-mt-24 rounded-lg border border-amber-400/25 bg-[#181a1d] p-6 shadow-sm">
...
function isTransfer(value: unknown): value is Transfer {
  return isRecord(value) && typeof value.id === 'string' && typeof value.serverId === 'string' && typeof value.serverName === 'string'
