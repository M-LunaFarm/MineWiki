  ...
  try {
    ...
    const metrics = await page.evaluate<RenderMetrics, { theme: string; minTargetSize: number }>((auditOptions) => {
      ...
      return {
        ...
        hasDocumentShell: visible('.wiki-shell.skin-article, .wiki-shell.skin-space'),
    ...
    if (!metrics.hasIntentStrip && !metrics.hasDocumentShell) fail(label, `missing ${options.requireDesktopNav ? 'visible ' : ''}page intent strip`);
