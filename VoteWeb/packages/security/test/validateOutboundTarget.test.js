...
import { UnsafeEndpointError, validateOutboundTarget } from '../src/index';
...
test('rejects loopback IPv4 target', async () => {
    await assert.rejects(() => validateOutboundTarget('127.0.0.1', 25565), (error) => error instanceof UnsafeEndpointError &&
        error.reason === 'private_address' &&
        error.message.includes('127.0.0.1'));
});
test('rejects RFC1918 private IPv4 target', async () => {
    await assert.rejects(() => validateOutboundTarget('192.168.1.10', 25565), (error) => error instanceof UnsafeEndpointError &&
        error.reason === 'private_address' &&
        error.message.includes('192.168.1.10'));
});
test('rejects metadata service IPs', async () => {
    await assert.rejects(() => validateOutboundTarget('169.254.169.254', 80, { label: 'Metadata endpoint' }), (error) => error instanceof UnsafeEndpointError &&
        error.reason === 'private_address' &&
        error.message.includes('169.254.169.254'));
});
test('rejects hosts containing path separators', async () => {
    await assert.rejects(() => validateOutboundTarget('example.com/..', 25565), (error) => error instanceof UnsafeEndpointError &&
...
test('rejects ports outside allowlist', async () => {
    await assert.rejects(() => validateOutboundTarget('8.8.8.8', 25566, { allowedPorts: [25565] }), (error) => error instanceof UnsafeEndpointError &&
