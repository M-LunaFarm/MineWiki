...
export function createDigestContent(
  ...
  const embed: APIEmbed = {
    title: options.title ?? 'CreeperVote Daily Digest',
    description,
    color: 0x4338ca,
    footer: {
  ...
  const content = options.roleRewardId
    ? `<@&${options.roleRewardId}> Daily CreeperVote digest is ready.`
    : undefined;
...
export function createPreviewContent(summary: DigestSummary): DigestContent {
  ...
  return createDigestContent(summary, {
    timezone: 'Asia/Seoul',
    nextDigestAt,
    title: 'CreeperVote Daily Digest (Preview)'
  });
}
  ...
  if (!subscription?.roleRewardId) {
    return {
      embeds: [
        {
          title: 'Role rewards',
          description: 'No reward role configured yet. Use /creepervote subscribe to set one.',
          color: 0x64748b
        }
      ]
    };
