...
export class EmailService {
  ...
  async sendVerificationEmail(payload: {
    ...
    const subject = 'CreeperVote email verification';
    const expiresAt = payload.expiresAt.toISOString();
    const text = [
      'Your CreeperVote verification code:',
      payload.token,
      '',
  ...
  async sendPasswordResetEmail(payload: {
    ...
    const subject = 'CreeperVote password reset';
    const expiresAt = payload.expiresAt.toISOString();
    const lines = [
      'Your CreeperVote password reset code:',
      payload.token,
      '',
